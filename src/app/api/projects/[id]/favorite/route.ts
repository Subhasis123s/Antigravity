...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    const updated = await ProjectService.toggleFavorite(user.id, projectId);
    ...
    logger.error("POST /api/projects/[id]/favorite error", err);
    return apiErrorResponse(err?.message || "Internal server error while toggling favorite.", 500);
  }
}
